...
### 3c. Get the token and open the browser (on the login node, then your browser)
...
Open in your browser: `http://localhost:8888/?token=<token>`
...
account with broad permissions, all of these are implicitly covered.
If you are operating under a restricted IAM role, attach the
`cluster-user-iam.yaml` stack output policy (or an equivalent inline policy)
