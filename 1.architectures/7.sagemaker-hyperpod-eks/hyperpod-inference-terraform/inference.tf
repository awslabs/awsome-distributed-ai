...
resource "kubernetes_manifest" "inference_endpoint" {
  manifest = {
    ...
    spec = {
      ...
      # NOTE: dnsConfig is intentionally omitted. When set, the operator creates
      # its own Route 53 A-record for var.domain_name pointing at the *internal*
      # ALB (private IPs), which conflicts with the public CloudFront alias
      # record managed in cloudfront.tf. CloudFront owns the public DNS; the
      # operator still attaches the ACM cert to the ALB via customCertificateConfig.
