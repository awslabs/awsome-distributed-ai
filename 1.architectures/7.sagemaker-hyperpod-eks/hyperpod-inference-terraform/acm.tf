...
# ============================================================================
# CloudFront only accepts ACM certs from us-east-1. For the same domain in the
# same account, ACM issues an identical DNS validation record, so the
# aws_route53_record.cert_validation records above validate this cert too.
resource "aws_acm_certificate" "cloudfront" {
  ...
  tags = {
    Name      = "hyperpod-inference-cf-${var.endpoint_name}"
    ManagedBy = "terraform"
  }
}
