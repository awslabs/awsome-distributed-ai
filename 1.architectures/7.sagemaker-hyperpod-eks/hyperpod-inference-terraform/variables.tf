...
variable "api_keys" {
  description = "List of API keys validated at the CloudFront edge (header x-api-key or Authorization: Bearer). No AWS credentials required for callers."
  type        = list(string)
  sensitive   = true
  default     = []
}
...
variable "cloudfront_origin_read_timeout" {
  description = "CloudFront origin read (response) timeout in seconds for long-running generations. Values above 60 require an approved CloudFront service quota increase for 'Response timeout per origin'."
  type        = number
  default     = 60
}
