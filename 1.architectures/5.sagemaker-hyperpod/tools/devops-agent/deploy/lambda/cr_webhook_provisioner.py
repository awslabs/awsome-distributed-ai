...
def _find_event_channel_association(agent_space_id: str) -> dict | None:
    ...
    list_associations is paginated; follow nextToken so an association past the
    first page isn't missed (mirrors _find_asset_id in cr_skill_uploader.py).
    """
    kwargs = {"agentSpaceId": agent_space_id}
    while True:
        resp = _devops.list_associations(**kwargs)
...
def _find_event_channel_service() -> str | None:
    ...
    list_services is paginated; follow nextToken so the service isn't missed on
    an account with more services than fit one page.
    """
    kwargs: dict = {}
    while True:
        resp = _devops.list_services(**kwargs)
...
def _provision(agent_space_id: str, secret_arn: str) -> dict:
    ...
    if existing is not None:
        ...
        # Do NOT return the webhook URL in the CR Data: it is a credential-bearing
        # endpoint whose home is Secrets Manager (written above). CR Data is logged
        # by the cfnresponse module and readable via DescribeStacks, so returning
        # it would leak the endpoint to two surfaces broader than the secret.
        return {"AssociationId": assoc_id, "ServiceId": svc_id}
    ...
    # See the note in the reuse branch above: the webhook URL is deliberately
    # NOT returned in the CR Data (Secrets Manager is its only home).
    return {"AssociationId": assoc_id, "ServiceId": svc_id}
...
def _event_channel_in_use_elsewhere(this_space_id: str) -> bool:
    ...
    """
    # list_agent_spaces is paginated; walk every page so a sibling space on a
    # later page can't be missed (which would wrongly deregister the singleton).
    spaces: list[dict] = []
    kwargs: dict = {}
    try:
        while True:
            resp = _devops.list_agent_spaces(**kwargs)
            spaces.extend(resp.get("agentSpaces", []) or [])
            token = resp.get("nextToken")
            if not token:
                break
            kwargs["nextToken"] = token
