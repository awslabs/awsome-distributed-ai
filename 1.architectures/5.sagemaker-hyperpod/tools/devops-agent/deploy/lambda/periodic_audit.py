...
def _k8s_list_all(path: str, eks_cluster_name: str, region: str,
    ...
    for _ in range(safe_max_pages):
        ...
        # Loop exhausted the runaway guard with a continue token still set:
        # the list is truncated. Surface it so a capped audit isn't mistaken
        # for full cluster coverage.
        if continue_token:
            _log("WARN", "k8s_list_truncated", path=path, pages=safe_max_pages,
                 page_limit=page_limit, items=len(items))
    return items
    ...
    for node in nodes:
        ...
        for cond in node.get("status", {}).get("conditions", []) or []:
            if cond.get("type") == "Ready":
                ready = cond
                break
        # A node that posted NO Ready condition (ready is None) is treated as
        # NotReady, not skipped — a node wedged early in registration is arguably
        # the least ready in the fleet. Only a node explicitly Ready=True is
        # skipped. (Mirrors the missing-lastTransitionTime fail-open below.)
        if ready is not None and ready.get("status") == "True":
            continue
        since = _parse_k8s_time(ready.get("lastTransitionTime", "")) if ready else None
...
def _load_webhook_credentials(force_refresh: bool = False) -> tuple[str, str]:
    """Fetch webhook URL + HMAC secret, cached per cold start.

    force_refresh bypasses (and repopulates) the cache — used to recover from a
    stale HMAC key after the provisioner re-associates the webhook while this
    sandbox stays warm (see the 401/403 retry in lambda_handler).
    """
...
def _build_payload(cluster_name: str, k8s_checks: dict | None,
    ...
    if heartbeat and not detected_issues:
        description = (
            ...
            f"Triage will typically SKIP it (the skipped task is itself the "
            f"console liveness signal); if it does PROCEED, the hyperpod-incident-rca "
            f"skill should emit 'Suppress — periodic audit, no open incidents'. "
            f"Either way it is visible in the console and never emailed."
        )
