...
This test case runs GRPO post-training with [**miles**](https://github.com/radixark/miles) on [Amazon SageMaker HyperPod](https://aws.amazon.com/sagemaker/hyperpod/) with Amazon EKS orchestration, or on a plain Amazon EKS cluster. It mirrors the sibling [`examples/training/slime/`](../slime/) test case: build a container image, prepare data, deploy a multi-node Ray cluster, convert model checkpoints, and launch GRPO training on NVIDIA GPUs interconnected with Elastic Fabric Adapter networking, EFA. miles is a fork of SLIME built for CUDA 13 and NVIDIA Blackwell as first-class hardware.
...
- [Sibling test case: examples/training/slime/](../slime/)
