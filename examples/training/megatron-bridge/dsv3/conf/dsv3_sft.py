...
"""Megatron-Bridge full-parameter SFT config for DeepSeek-V3 (671B MoE) on 256x B300.

This module builds and returns a Megatron-Bridge ``ConfigContainer`` for supervised
fine-tuning (full parameter, BF16) of DeepSeek-AI's DeepSeek-V3 on an EKS cluster of
...
This is the SFT sibling of the dispatcher A/B benchmark in ``../benchmarks/`` — same
model family, same image, same UCCL-over-EFA dispatch path — and the parallel of
``../../kimi-k2/conf/kimi_k2_sft.py`` (DeepSeek-V3 is the 256-expert architecture family
that the literal 384-expert Kimi-K2 belongs to).
...
Usage (inside the container, launched by torchrun on all 256 ranks):
    ...
    from conf.dsv3_sft import dsv3_sft_config

    finetune(config=dsv3_sft_config(), forward_step_func=forward_step)

Grounded against Megatron-Bridge ``main`` (image ships v0.4.2):
...
- HF->MCore bridge : src/megatron/bridge/models/deepseek/deepseek_v3_bridge.py (dedicated)
...
# unverified against Bridge v0.4.2 — the moe_token_dispatcher_type/moe_flex_dispatcher_backend
...
# ---------------------------------------------------------------------------
FSX_ROOT = os.environ.get("FSX_ROOT", "/fsx/dsv3")

# Megatron-Core distributed checkpoint produced by the HF -> MCore conversion step
# (Megatron-Bridge AutoBridge.import_ckpt; see ../../convert-checkpoint.sh / README).
# BF16, ~1.3 TB.
PRETRAINED_CHECKPOINT = os.environ.get(
    "DSV3_MCORE_CKPT", os.path.join(FSX_ROOT, "mcore")
)
# SFT training output (saved torch_dist checkpoints + resume).
OUTPUT_DIR = os.environ.get("DSV3_OUTPUT_DIR", os.path.join(FSX_ROOT, "sft-output"))
# Tokenizer: the HF DeepSeek-V3 repo dir on FSx (config + tokenizer). We pass a local
# path so no network/auth is needed on the workers.
HF_MODEL_PATH = os.environ.get("DSV3_HF_PATH", os.path.join(FSX_ROOT, "hf"))
...
DSV3_HF_ID = os.environ.get("DSV3_HF_ID", "deepseek-ai/DeepSeek-V3-Base")
...
# EP=32 spans TP*DP = 32; 256 routed experts / 32 = 8 experts per EP rank.
# ---------------------------------------------------------------------------
...
def dsv3_sft_config() -> ConfigContainer:
    """Return a full-parameter SFT ``ConfigContainer`` for DeepSeek-V3 on 256x B300.

    Parallelism (256 GPUs, expert_tensor_parallel_size=1):
        TP=8, PP=8, DP=4  ->  world = TP*PP*DP = 8*8*4 = 256.
        DP = world/(TP*PP) = 256/64 = 4.
        EP=32             ->  EP spans TP*DP = 32; 256 routed experts / 32 = 8 per EP rank.
                              EP must divide TP*DP = 32 with ETP=1 -> 32 | 32 OK.
        CP=1.
    Memory sanity: ~16 B/param (BF16 weights+grads + FP32 Adam moments, distributed
    optimizer) x 671B / 256 ~= 42 GB/GPU of sharded model state, leaving headroom on
    ...
    (higher activation memory — exercised empirically by the dsv3 benchmark cells).
    """
    ...
    # versions; pinned image is Bridge v0.4.2.
    cfg = _sft_common()

    # -- Model provider from the DeepSeek-V3 architecture --------------------
    ...
    # deepseek-ai/DeepSeek-V3-Base declares architectures=["DeepseekV3ForCausalLM"], which
    # AutoBridge routes to the registered DeepSeekV3Bridge (deepseek_v3_bridge.py — a
    # dedicated bridge, unlike Kimi-K2 which rides the same DeepSeek-V3 family path),
    # building the literal DeepSeek-V3 provider (256 experts, 128 heads, MLA, 61 layers,
    # 1 MTP layer) straight from the HF config.
    # trust_remote_code=True: the DeepSeek-V3 HF config uses auto_map ->
    # configuration_deepseek.DeepseekV3Config (custom code); keep it on unless the image's
    # transformers registers DeepseekV3Config natively (harmless when it does).
    cfg.model = AutoBridge.from_hf_pretrained(
        DSV3_HF_ID, trust_remote_code=True
    ...
    cfg.model.expert_model_parallel_size = EXPERT_PARALLEL    # 256 % 32 == 0
    cfg.model.expert_tensor_parallel_size = 1
    cfg.model.context_parallel_size = 1
    cfg.model.sequence_parallel = True                # required with TP>1 for MoE/MLA
    cfg.model.pipeline_dtype = torch.bfloat16
    # DeepSeek-V3 ships ONE multi-token-prediction layer (HF num_nextn_predict_layers=1),
    # unlike Kimi-K2 which has none. So the AutoBridge provider is EXPECTED to yield
    # mtp_num_layers=1 from the HF config; KEEP it — do NOT null it (that would drop the MTP
    # head the converted checkpoint carries). mtp=1 satisfies core's comm-overlap assert
    # (`mtp_num_layers is None or == 1` when overlap_moe_expert_parallel_comm is enabled),
    # and the DSV3 pipeline-layout helper is MTP-aware (it appends an "mtp" entry to the
    # last stage).
    # ASSUMPTION (validate against image): the dsv3 benchmark validated the layout helper for
    # PP=8 via the *recipe* provider (deepseek_v3_pretrain_config_32nodes), whose mtp value is
    # the recipe's; this SFT path builds the provider via AutoBridge+HF config, a DIFFERENT
    # source. kimi_k2_sft.py exercised the helper with mtp=None — changing the load-bearing
    # input to mtp=1 here is not separately confirmed. It is GUARDED, not silent: an
    # unexpected MTP depth raises below, and a missing (PP=8, mtp=1) layout trips the
    # `pipeline_model_parallel_layout is None` check further down (fails loud at build).
    if cfg.model.mtp_num_layers not in (None, 1):
        raise ValueError(
            "DeepSeek-V3 provider yielded mtp_num_layers=%r; expected 1 (HF "
            "num_nextn_predict_layers=1). The MTP-aware pipeline layout and the overlap "
            "assert require mtp_num_layers in {None, 1}." % cfg.model.mtp_num_layers
        )
    ...
    # See ../benchmarks/RESULTS.md for the A/B methodology.
    ...
    if moe_dispatcher == "alltoall":
        ...
        # v0.4.2 / MCore on the image (the recipes use moe_token_dispatcher_type +
    ...
    # exposes the A2A fully (dispatcher-isolation upper bound). See ../benchmarks/RESULTS.md.
    ...
    # an unvalidated follow-up (see ../benchmarks/RESULTS.md).
    ...
    # mirroring the validated benchmark entrypoint (../benchmarks/bench_dsv3_pretrain.py).
    if moe_a2a_overlap and PIPELINE_PARALLEL > 1:
        ...
        cfg.model.recompute_modules = None
    # DeepSeek-V3's 61 layers do NOT divide evenly over PP=8 — an explicit layout is REQUIRED
    # (None errors at model build). Reuse the DSV3 recipe helper: it is MTP-aware (mtp=1
    # preserved above -> last stage ["mtp","loss"]) and ships layouts for (pp,vpp) in
    # {(4,1),(8,1),(16,1),(4,2),(8,2),(4,4)}. The dsv3 benchmark exercised this helper for
    # PP=8 via the recipe provider; here the provider (and thus its mtp value) comes from
    # AutoBridge instead, so the None-check below is the authoritative gate (see the MTP
    ...
    # local SFT corpus on Bridge v0.4.2. The FinetuningDatasetConfig / HFDatasetConfig
    ...
    SFT_DATASET_DIR = os.environ.get("DSV3_SFT_DATA", os.path.join(FSX_ROOT, "sft-data"))
...
if __name__ == "__main__":
    ...
    cfg = dsv3_sft_config()
    print("DeepSeek-V3 SFT ConfigContainer built.")
    print(
    ...
    # symbol are unverified against Bridge v0.4.2. The shape below mirrors the
