...
# DeepSeek-V3 Full-Parameter SFT on EKS — Kubernetes Deployment

This directory contains the manifests and instructions for launching a 32-node (256-GPU)
Megatron-Bridge SFT job for DeepSeek-V3 (671B-param MoE, 256 routed experts) on Amazon EKS
with UCCL-EP providing expert-parallel all-to-all over AWS EFA.
...
> [`../../run-ab-rawpods.sh`](../../run-ab-rawpods.sh)). The full SFT
> additionally needs the ~1.3 TB MCore checkpoint at `/fsx/dsv3/mcore` (absent at the time
...
> verification gates in [Section 8](#8-confirm-efa--not-ib--nvshmem--is-carrying-ep-traffic)
> (§7.1–7.4), by contrast, were captured from live raw-pod runs and are accurate.
...
- **Container image** built and pushed to ECR — this is the shared, model-agnostic
  ...
  `examples/training/megatron-bridge/README.md` (`bash 1.build-and-push.sh`).
...
DeepSeek-V3 checkpoints require approximately 3–4 TB of scratch space
(~0.7 TB HF weights + ~1.3 TB Megatron-Core BF16 distributed checkpoint + working headroom).
Provision a filesystem of at least **3600 Gi** and then create a PersistentVolume and
...
export EXPERT_PARALLEL=32              # EP=32 spans TP*DP=32; divides 256 routed experts exactly (8/rank)
...
`conf/dsv3_sft.py` / baked into the manifest env and are not exported here.

## 4. Create the SFT-config ConfigMap

The shared env image is **model-agnostic** and does **not** bake in the SFT config.
The PyTorchJob mounts `conf/dsv3_sft.py` at `/workspace/conf` from a ConfigMap named
`dsv3-sft-conf`. Create it from the conf file in the model directory (one level up
from `kubernetes/`):

```bash
cd examples/training/megatron-bridge/dsv3

# Create (or re-create) the ConfigMap from the SFT config:
kubectl create configmap dsv3-sft-conf \
  --from-file=dsv3_sft.py=conf/dsv3_sft.py \
  --dry-run=client -o yaml | kubectl apply -f -

kubectl get configmap dsv3-sft-conf   # confirm it exists before applying the job
```

Re-run this whenever you edit `conf/dsv3_sft.py` (then restart the job to pick it up).
The ~17 KB config is well under the 1 MiB ConfigMap limit.

## 5. Render and Apply the Manifest

```bash
cd examples/training/megatron-bridge/dsv3/kubernetes

envsubst < manifests/dsv3-sft-pytorchjob.yaml-template | kubectl apply -f -
```
...
| `dsv3-sft` | PyTorchJob | 32 Worker replicas (mounts `dsv3-sft-conf` at `/workspace/conf`) |
...
`manifests/dsv3-sft-pytorchjob.yaml-template` and apply the `PodGroup` in the same
...
kubectl get pods -l job-name=dsv3-sft -w
...
kubectl get podgroup dsv3-sft-pg
...
dsv3-sft-worker-0     1/1     Running   0          1m
dsv3-sft-worker-1     1/1     Running   0          1m
...
dsv3-sft-worker-31    1/1     Running   0          1m
```
...
kubectl logs -f dsv3-sft-worker-0

# Or watch all workers in parallel with stern:
stern dsv3-sft-worker --namespace default
```
...
```bash
kubectl get pytorchjob dsv3-sft -o yaml
...
```bash
kubectl exec -it dsv3-sft-worker-0 -- python3 -c \
...
```bash
kubectl logs dsv3-sft-worker-0 | grep -i "NET/OFI\|provider\|efa"
```
...
```bash
kubectl logs dsv3-sft-worker-0 | grep -i "nvshmem\|IBGDA"
...
```bash
kubectl logs dsv3-sft-worker-0 | grep -i "uccl\|deep_ep\|efa.*bw\|all2all"
```
...
envsubst < manifests/dsv3-sft-pytorchjob.yaml-template | kubectl delete -f -

# Verify pods are gone
kubectl get pods -l job-name=dsv3-sft
