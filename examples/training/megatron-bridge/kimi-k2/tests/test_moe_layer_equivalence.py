...
def elastic_moe(inputs, indices, probabilities, expert_scale, experts):
    buffer = get_elastic_buffer(dist.group.WORLD, inputs.shape[0], inputs.shape[1], indices.shape[1])
    ...
    def observe_dispatch_backward(gradients):
        grad = gradients[2]
        print("DISPATCH_PROBABILITY_GRADIENT " + json.dumps({
            "rank": dist.get_rank(), "shape": list(grad.shape),
            "stride": list(grad.stride()), "dtype": str(grad.dtype),
            "is_cuda": grad.is_cuda, "is_contiguous": grad.is_contiguous(),
            "scope": "Gate B TE-fused permute/unpermute adjoint, no gradient modification",
        }), flush=True)
        # Returning None leaves the original autograd gradients unchanged.
    recv_probabilities.grad_fn.register_prehook(observe_dispatch_backward)
    ...
    output, _ = deepepv2_combine(buffer, local, handle)
