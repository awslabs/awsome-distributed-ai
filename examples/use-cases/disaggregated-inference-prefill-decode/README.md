...
The shape-A result at 0.1 tasks/s is **not reliably reproducible**, so treat a miss there as a valid observation rather than a fault. Six consecutive phases on one engine, using the same traffic file and seed, produced p90 TTFT of `1024.4`, `4072.2`, `956.8`, `1035.1`, `13 896.3` and `919.8` ms; two of the six missed the objective, and the failures fell in both the warmup and measured phases. p90 TPOT stayed within `21.4`–`21.7` ms throughout, `completed_calls` equalled `sent_calls` everywhere, and `client_valid` was true, so the spread is server-side TTFT only. Within a single failing phase the values were bimodal — twelve calls near `0.8 s` and nine between `3.9` and `4.1 s`, in contiguous blocks — rather than a decaying warmup curve.

One periodic disturbance is confirmed to exist but is not confirmed to be the cause. `SGLANG_ENABLE_HEALTH_ENDPOINT_GENERATION` defaults to true in the pinned engine, so the `/health` path both probes share runs a real generation, and each `GET /health` on an idle engine is preceded by a scheduler prefill batch at the probe period of ten seconds. In disaggregated mode that request carries a fake bootstrap host. Whether it accounts for the observed TTFT blocks was not tested: doing so requires setting `SGLANG_DIAG_BYPASS_HEALTH_GENERATE`, which the pinned engine documents for exactly this case, and re-measuring. Until that experiment is run, record your own `meets_joint_slo` for shape A instead of expecting a fixed value.
