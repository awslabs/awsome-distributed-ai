...
That window belongs to the router, not the engines. After an engine pod restarts while the router keeps running, the router answers its own `/health` with 200 while `POST /generate` returns 503 `No available decode workers (all circuits open or unhealthy)` — measured for about 90 s *after* `kubectl rollout status` reported both engine deployments Ready, with both engines idle throughout. Since `deployment.py` runs the router with `--disable-retries`, one such 503 fails a whole agent task. `4.ramp.py` now waits for `/generate`, not just `/health`, before each rate; after any engine restart let the router settle rather than measuring immediately. The engine's own periodic `/health` generation is visible in the prefill worker's log at the ten-second probe period and was measured *not* to affect this — `SGLANG_DIAG_BYPASS_HEALTH_GENERATE` removes it and changes nothing.
