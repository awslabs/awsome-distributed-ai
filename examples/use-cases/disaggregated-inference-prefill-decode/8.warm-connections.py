...
def main():
    ...
    prefill = [x for x in pods if x["metadata"]["labels"]["role"] == "prefill"]
    decode = [x for x in pods if x["metadata"]["labels"]["role"] == "decode"]
    assert prefill and len(decode) == 1, "Fixed round-robin P/D topology required"
    r = asyncio.run(warm(c, a.url, len(prefill)))
    flush = {}
    for pod in pods:
        code = "import urllib.request; r=urllib.request.urlopen('http://127.0.0.1:30000/flush_cache'); print(r.status, r.read().decode())"
        flush[pod["metadata"]["name"]] = kubectl(c, "exec", pod["metadata"]["name"], "-c", "engine", "--", "python3", "-c", code, capture_output=True).stdout
    out = Path(a.output)
