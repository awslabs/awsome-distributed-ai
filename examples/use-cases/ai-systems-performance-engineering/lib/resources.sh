...
python3 - "$count" "$source_name" "$cores" "${SLURM_CPUS_ON_NODE:-}" <<'PY'
import json, os, socket, sys
gpus, source, cores, slurm_cpus = sys.argv[1:]
print(json.dumps(dict(host=socket.gethostname(), gpus=int(gpus), gpu_count_source=source,
                     available_processors=int(cores), slurm_cpus_on_node=slurm_cpus,
