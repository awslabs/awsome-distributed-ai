...
> `S3_BUCKET_NAME`, `RAY_ADDRESS` and `MLFLOW_TRACKING_NAME`.
...
### The FSx volume is a prerequisite, not part of this test case

Provisioning FSx for Lustre belongs to the architecture layer. Use
[`1.architectures/7.sagemaker-hyperpod-eks`](../../../../../../1.architectures/7.sagemaker-hyperpod-eks)
and its
[`fsx_lustre` Terraform module](../../../../../../1.architectures/7.sagemaker-hyperpod-eks/terraform-modules/hyperpod-eks-tf/modules/fsx_lustre),
which creates the filesystem and the StorageClass / PV / PVC for you.

What this test case requires is only that the result is reachable:

```bash
kubectl get pvc fsx-lustre -n "${KUBE_NAMESPACE}"   # must exist and be Bound
```

Two things that will silently not work if the volume was provisioned by hand instead:

- **`mountname` is not the filesystem id.** It is `LustreConfiguration.MountName`, a short
  opaque string. Using the `fs-...` id mounts nothing.
...
./models/run_on_cluster.sh models/download_model.sh Qwen/Qwen3-Coder-Next
./models/run_on_cluster.sh models/download_model.sh Qwen/Qwen2.5-Coder-7B-Instruct
./models/run_on_cluster.sh models/download_model.sh Qwen/Qwen3-30B-A3B
```
