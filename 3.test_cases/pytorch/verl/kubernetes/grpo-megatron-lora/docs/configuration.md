...
| `data` | `eurus`, `mixed`, `mixed-hard`, `mixed-valplus` | `eurus` | Train/val file paths, prompt key, data name for checkpoint separation |
| `sandbox` | `enabled`, `disabled` | `enabled` | SandboxFusion URL and per-execution memory limit |
| `tracking` | `mlflow`, `console` | `mlflow` | Experiment loggers (MLflow, console) |
| `profiling` | `disabled`, `torch`, `nsys`, `memory` | `disabled` | Profiler tool, steps, output path |
...
GPU count, offloading and the shared-storage root are **not** a config group. They are `compute.*`
keys in `conf/config.yaml`, because they are parameters verl needs on its command line rather than a
description of your hardware — provisioning the cluster is out of scope for a test case (see the
README Prerequisites). Override them the same way:

```bash
python3 scripts/submit_training.py compute.num_nodes=1 compute.agent_num_workers=4
```
...
```
conf/
├── config.yaml                  # Root config: compute, training hyperparams, Ray, secrets, defaults
├── backend/
...
│   ├── mixed-hard.yaml          # Difficulty-filtered train split (treatment arm)
│   └── mixed-valplus.yaml       # Enlarged val split (see the contamination note in the file)
├── sandbox/
...
└── profiling/
    ├── disabled.yaml            # No profiling (default)
    ├── torch.yaml               # torch profiler on selected steps
    ├── nsys.yaml                # Nsight Systems
    └── memory.yaml              # CUDA memory snapshots
```
...
python3 scripts/submit_training.py \
  compute.param_offload=true \
  compute.optimizer_offload=true \
  compute.gpu_memory_utilization=0.6
...
| `compute.gpu_memory_utilization` | `0.85` | `0.85` | vLLM rollout memory fraction; safe on B200 ([verl perf tuning](https://verl.readthedocs.io/en/latest/perf/perf_tuning.html)) |
| `compute.param_offload` | `false` | `false` | Not needed -- 7B + LoRA fits in GPU memory easily |
| `compute.optimizer_offload` | `false` | `false` | |
...
| `compute.gpu_memory_utilization` | `0.7` | `0.7` | Lower than LoRA -- more GPU memory needed for training state |
| `compute.param_offload` | `false` | `false` | B200 has enough memory for 7B full fine-tuning |
| `compute.optimizer_offload` | `false` | `false` | Enable if you see OOM during actor update |
...
python3 scripts/submit_training.py \
  ...
  compute.gpu_memory_utilization=0.7
```
...
| `compute.gpu_memory_utilization` | `0.5` | `0.6` | Lower on 2-node since training state competes for memory ([verl perf tuning](https://verl.readthedocs.io/en/latest/perf/perf_tuning.html)) |
| `compute.param_offload` | `false` | `false` | LoRA adapters are small; base model weights are frozen |
| `compute.optimizer_offload` | `false` | `false` | Only LoRA params have optimizer states |
...
python3 scripts/submit_training.py \
  model=qwen25-72b \
  lora.rank=64 lora.alpha=32 \
  compute.param_offload=true compute.optimizer_offload=true \
  compute.gpu_memory_utilization=0.4
```
...
| `compute.gpu_memory_utilization` | `0.4` | `0.6` | Low on 2-node; optimizer states consume most of GPU memory |
| `compute.param_offload` | `true` | `false` | Needed on 2-node; 6-node has enough aggregate memory |
| `compute.optimizer_offload` | `true` | `true` | Recommended even on 6-node for 72B full FT |
...
python3 scripts/submit_training.py \
  ...
  compute.optimizer_offload=true compute.gpu_memory_utilization=0.6
```
...
| `compute.gpu_memory_utilization` | `0.4` | `0.7` | |
| `compute.param_offload` | `true` | `false` | Required on 2-node |
| `compute.optimizer_offload` | `true` | `false` | |
| Ref model offload | `true` | `true` | |
...
| Scenario | `compute.param_offload` | `compute.optimizer_offload` | `backend.grad_offload` |
|----------|:-:|:-:|:-:|
...
**`compute.gpu_memory_utilization`** controls how much GPU memory vLLM claims during the
...
python3 scripts/submit_training.py \
  ...
  compute.gpu_memory_utilization=0.85
```
...
`submit_training.py` emits the first four for you on the FSDP path. Anything else in this list is a
verl flag you would pass yourself; it is not exposed as a config key here.
...
`models/submit_download.sh <HF_ID> <DEST>` (Ray job) and
`models/run_on_cluster.sh models/download_model.sh <HF_ID>` (fsx-utils pod) are both generic. The
model IDs matching each `conf/model/` group:

| Config group | HuggingFace model ID | Directory created under `${compute.fsx_home}/models/` |
|---|---|---|
| `qwen3-8b` | `Qwen/Qwen3-8B` | `Qwen3-8B` |
| `qwen25-coder-7b` | `Qwen/Qwen2.5-Coder-7B-Instruct` | `Qwen2.5-Coder-7B-Instruct` |
| `qwen25-72b` | `Qwen/Qwen2.5-72B-Instruct` | `Qwen2.5-72B-Instruct` |
| `qwen3-coder-next` | `Qwen/Qwen3-Coder-Next` | `Qwen3-Coder-Next` |
| `qwen3-30b-a3b` | `Qwen/Qwen3-30B-A3B` | `Qwen3-30B-A3B` |
| `qwen3-235b` | `Qwen/Qwen3-235B-A22B` | `Qwen3-235B-A22B` |

The directory name must match the `name` field in the corresponding `conf/model/*.yaml`, because
`fsx_path` is `${compute.fsx_home}/models/${model.name}`.

---

## Repository conventions

Useful if you are extending this test case.

**Shell scripts**

- Training/data scripts: `#!/usr/bin/env bash`; setup/build scripts: `#!/bin/bash`
- `set -euo pipefail`. Do **not** add `set -x` to a script that sources `env_vars` — xtrace applies
  inside the sourced file and would echo `HF_TOKEN` to stderr
...
- All env vars use `${VAR:-default}` — never assume a var is set
- Naming: underscores in `scripts/` and `data/`, hyphens at the root

**Dockerfile**

- Base image `verlai/verl:${BASE_TAG}` — pin to an immutable tag, never `:latest`
- Layer order: system deps -> EFA -> blinker fix -> Megatron-Bridge (`--no-deps`)
- verl, transformers and pip extras go in `scripts/runtime_env.yaml`, not the Dockerfile, so a
  dependency bump does not require an image rebuild
- Env vars the image must set: `CUDA_DEVICE_MAX_CONNECTIONS=1`, `NCCL_NVLS_ENABLE=0`,
  `VLLM_USE_V1=1`
- `scripts/custom_reward_fn.py` is `COPY`ed to `/workspace/custom_reward_fn.py` in the image, which
  is where the Ray workers read it from
