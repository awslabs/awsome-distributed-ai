...
- **A larger code eval set (BigCodeBench, n=1140) was attempted and found NOT usable with this
  harness.** A completion-mode task definition was written against the `utils.py` builders in
  `kubernetes/lmeval-tasks/`, but a canonical-solution HARD GATE proved lm-eval's `code_eval` executor is
  ...
  reservation task. No BigCodeBench model number was produced, and the non-functional task YAML is
...
## Sandbox concurrency and OOM (unresolved)

The code-reward sandbox OOM-kills itself under load, and the knob that looks like the fix is not
the fix. This is recorded here rather than in `conf/sandbox/enabled.yaml` because the conclusion is
"do not turn this knob", and because the sandbox is still an open issue.

**The knob.** verl's sandbox path accepts a `concurrent_semaphore` and, when given one, wraps every
execute-API call in it. It defaults to `None` — unbounded — so `scripts/custom_reward_fn.py`
constructs one, sized by `SANDBOX_MAX_CONCURRENT_PER_WORKER` (default 5). There is no verl CLI
override for it, which is why nothing is plumbed from the Hydra config.

**The first model, which was wrong.** The original reasoning was arithmetic on pod memory: 5 pods
at a 48Gi limit, each in-flight request allowed `memory_limit_mb=1024`, giving ~51.2 concurrent per
pod × 1024 MB = 51.2 GiB > 48 GiB → `exitCode 137`. On 2026-08-04 all 5 pods were OOMKilled 24–30
times with ~13 min between restarts and 290 "Max retries" in the reward path — while
`val_before_train` was measuring the **baseline**. A flapping scorer silently biases CODE downward,
which is the class of bug that produced a retracted CODE gain.

**Why it is wrong.** A later run held the semaphore at 10 for its whole 9.9 h life and all five pods
were *still* OOMKilled 6–7 times each (466 "Max retries"), even though that arithmetic predicts only
33% of the limit. Measured from `[SANDBOX-STATS]` per-source counts on that run: 77,654 test-case
outcomes across 2,000 code samples = **~39 test cases per request** (taco worst at 52,252 outcomes).
`memory_limit_mb` caps one **execution**; the semaphore caps in-flight **requests**; nothing bounds
concurrent executions, which is the quantity that actually consumes pod memory. That is why 16 → 10
changed nothing.

Two hypotheses ruled out at the same time:

- **Not validation-driven.** The 2026-08-06 OOMs landed six hours into pure training with no
  validation running (`test_freq=50` was never reached), and the training-window error density
  (5.66 per 1k lines) was *higher* than the baseline-validation window (4.14 per 1k).
- **Not a slow leak.** All five pods died within 36 seconds of each other — a coordinated load
  spike fanning out across the 5-pod Service, not five processes independently drifting into a limit.

**Lowering it further does not help, and the negative result is the useful part.** 10 → 5 was tried
as the single conservative change. The OOM was **not** fixed: all 5 pods still OOMKilled in a
synchronized 13-second burst (3 restarts each) at a *higher* restart rate than at 10 (~1.3 vs ~0.67
per pod per hour). What it did buy was a **5.0× drop in sandbox error density** (2.77 vs 13.89 per 1k
log lines, identical regex), because fewer in-flight requests are destroyed when a pod dies. That is
exactly what the requests-vs-executions diagnosis predicts: the semaphore bounds **blast radius**,
not peak memory.

**What is not available as a fix.** Raising pod memory limits or adding replicas is unsafe: the pods
share a node with `ray-head`, and pushing that node over its memory limits risks evicting `ray-head`
and killing the training job outright. Lowering `memory_limit_mb` caps the memory available to the
candidate program, so it makes legitimate solutions fail and **changes scores** — as would capping
test cases per request. Neither may be touched while a run is measuring.

A real fix needs an execution-level bound that does not exist today — a second semaphore around the
per-test-case loop, or batching test cases into fewer sandbox calls — plus a scores-unchanged proof
before deployment.
