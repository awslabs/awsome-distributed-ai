...
# produced the retracted "+0.158 CODE gain".
#
# NOTE: `sandbox.max_concurrent` in conf/sandbox/*.yaml is DEAD CONFIG. It is
# referenced by nothing -- submit_training.py only plumbs sandbox_fusion_url and
# memory_limit_mb. Lowering it has no effect. This semaphore is the real knob.
#
# The semaphore is per PROCESS, and verl runs several RewardLoopWorkers (8 were
# observed on 6 hosts), so total concurrency is workers x this value:
#
#   8 workers x 16 = 128 in flight -> 128/5 pods = ~25.6/pod x 1024MB
#   = ~25.6 GiB per pod == ~53% of the 48Gi limit.
#
# Do NOT "fix" sandbox OOM by raising pod memory limits or adding replicas:
# the pods are pinned to the ml.m5.24xlarge that also hosts ray-head (64Gi
# limit), and that node is already at 92% of its memory limits. Pushing it over
...
# optimistic -- it measures the wrong quantity.
#
# Measured from [SANDBOX-STATS] per_source counts on that run: 77,654 test-case
# outcomes across 2,000 code samples = **~39 test cases PER REQUEST** (taco is
# worst: 52,252 outcomes). `memory_limit_mb=1024` caps the memory of a single
# EXECUTION, and this semaphore caps in-flight REQUESTS -- but one request fans
# out to ~39 executions. Nothing in the stack bounds concurrent executions, which
# is the quantity that actually consumes pod memory.
#
# Two further observations that rule out the earlier hypotheses:
#   - It is NOT validation-driven. The 2026-08-06 OOMs landed at 22:08-22:09 UTC,
#     six hours into pure training with no validation running (test_freq=50 was
#     never reached), and the training-window error density (5.66/1k lines) was
#     HIGHER than the baseline-validation window (4.14/1k). The "mixed-valplus
#     added +49% val rows" explanation is therefore falsified.
#   - It is NOT a slow leak. All five pods died within 36 seconds of each other,
#     which is a coordinated load spike fanning out across the 5-pod Service,
#     not five independent processes drifting into a limit.
#
# Lowered 10 -> 5 as the single conservative change. Rationale: it is the only
...
# identical regex), because fewer in-flight requests are destroyed when a pod dies.
#
# That is precisely what the requests-vs-executions diagnosis predicts: this semaphore
# bounds BLAST RADIUS, not peak memory. Peak memory is driven by the ~39 test-case
# executions fanning out per request, which nothing bounds. Lowering this further will
# keep buying error density and never fix the OOM. Do not keep turning this knob.
#
# The only knob that bounds a single EXECUTION is `memory_limit_mb`, but it caps the
# memory available to the candidate program, so lowering it makes legitimate solutions
# fail and CHANGES SCORES. Neither that nor a test-case cap may be touched while a run
# is measuring. A real fix needs an execution-level bound that does not exist today --
# e.g. a second semaphore around the per-test-case loop, or batching test cases into
# fewer sandbox calls -- plus a scores-unchanged proof before deployment.
