...
def _build_shared_overrides(cfg: DictConfig) -> list[str]:
    ...
    overrides = [
        ...
        # for expert routing headroom), falls back to cluster default.
        f"actor_rollout_ref.rollout.gpu_memory_utilization="
        f"{OmegaConf.select(cfg.model, 'gpu_memory_utilization', default=cfg.cluster.gpu_memory_utilization)}",
        ...
        f"trainer.n_gpus_per_node={cfg.cluster.gpus_per_node}",
        f"trainer.nnodes={cfg.cluster.num_nodes}",
        f"trainer.default_local_dir={cfg.cluster.fsx_home}/ckpts/{cfg.data_name}/{cfg.backend.name}/{cfg.model.name}",
...
def _build_fsdp_overrides(cfg: DictConfig) -> list[str]:
    ...
    overrides = [
        ...
        f"actor_rollout_ref.actor.fsdp_config.param_offload={cfg.cluster.param_offload}",
        f"actor_rollout_ref.actor.fsdp_config.optimizer_offload={cfg.cluster.optimizer_offload}",
        f"actor_rollout_ref.actor.fsdp_config.fsdp_size={cfg.cluster.gpus_per_node}",
        # Agent loop workers
        f"actor_rollout_ref.rollout.agent.num_workers={cfg.cluster.agent_num_workers}",
...
def _build_megatron_overrides(cfg: DictConfig) -> list[str]:
    ...
    overrides = [
        ...
        # even though the cluster defaults are False (sufficient for smaller models).
        f"actor_rollout_ref.actor.megatron.param_offload="
        f"{OmegaConf.select(cfg.model, 'param_offload', default=cfg.cluster.param_offload)}",
        f"actor_rollout_ref.actor.megatron.optimizer_offload="
        f"{OmegaConf.select(cfg.model, 'optimizer_offload', default=cfg.cluster.optimizer_offload)}",
...
def build_verl_overrides(cfg: DictConfig) -> list[str]:
    ...
    if cache_mode == "local":
        ...
        cache_root = str(Path(cfg.cluster.fsx_home).parent / "cache" / CACHE_STACK_VERSION)
    overrides.extend(
        [
...
def print_config_summary(cfg: DictConfig) -> None:
    ...
    print(
        f"  Cluster:   {cfg.cluster.num_nodes}x {cfg.cluster.instance_type} "
        f"({cfg.cluster.num_nodes * cfg.cluster.gpus_per_node} GPUs)"
    )
    if cfg.backend.name == "megatron":
...
def preflight_resume_check(cfg: DictConfig) -> None:
    ...
    ckpt_dir = f"{cfg.cluster.fsx_home}/ckpts/{cfg.data_name}/{cfg.backend.name}/{cfg.model.name}"
