...
#     RANK = this node's rank, 0..WORLD_SIZE-1 (rank 0 must be $MASTER_IP)
#
# The rendezvous master defaults to PREFILL_HEAD_IP; override with MASTER_IP to
# smoke-test any other pair of nodes (e.g. the decode role).
#
...
RANK=${2:?need RANK (0..WORLD_SIZE-1)}
shift 2

: "${IMAGE_URI:?source setup/env_vars first}"
: "${IFACE:?source setup/env_vars first}"
MASTER_IP=${MASTER_IP:-${PREFILL_HEAD_IP:?set PREFILL_HEAD_IP or MASTER_IP}}
WS=${WORLD_SIZE:-2}
...
docker run --rm \
    ...
    -e MASTER_ADDR="$MASTER_IP" -e MASTER_PORT="$PORT" \
