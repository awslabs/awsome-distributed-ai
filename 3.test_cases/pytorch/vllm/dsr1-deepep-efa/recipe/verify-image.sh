...
# Run this before any multi-node work — it catches the shadowing and
# version-collision failure modes described in the README, which otherwise
# surface as confusing NVSHMEM errors much later.
#
# Usage:
...
echo "--- NIXL LIBFABRIC plugin present (KV over EFA for PD) ---"
if [ -e "${NIXL_PLUGIN_DIR}/libplugin_LIBFABRIC.so" ]; then
    echo "OK: ${NIXL_PLUGIN_DIR}/libplugin_LIBFABRIC.so"
else
    echo "FAIL: no libplugin_LIBFABRIC.so under ${NIXL_PLUGIN_DIR:-<unset>}"; fail=1
fi
python3 -c "import nixl; print(\"OK: nixl imports\")" || fail=1

echo "--- mooncake imports (EFA build) ---"
python3 -c "import mooncake; print(\"OK: mooncake\", mooncake.__file__)" || fail=1

echo "--- vllm + ray import ---"
python3 -c "import vllm; print(\"OK: vllm\", vllm.__version__)" || fail=1
python3 -c "import ray; print(\"OK: ray\", ray.__version__)" || fail=1
