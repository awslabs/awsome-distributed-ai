...
ENVSUBST_VARS_FULL='$IMAGE_URI $NAMESPACE $FSX_PVC_NAME $NUM_NODES $GPU_PER_NODE $EFA_PER_NODE $DOMAINS $DATA_DIRS $NORM_STATS_PATH $PTV3_SIZE $BATCH_SIZE $NUM_WORKERS $EVAL_FREQ $SAVE_FREQ $LOG_DIR $EXP_NAME $WANDB_MODE $MODEL_PATH $EVAL_NUM_BATCHES'
