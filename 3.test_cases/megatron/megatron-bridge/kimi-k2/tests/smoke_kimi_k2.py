...
def main() -> None:
    ...
    benchmark = pathlib.Path(__file__).parents[1] / "benchmarks" / "bench_kimi_k2_pretrain.py"
    ...
    assert (model.moe_token_dispatcher_type, model.moe_flex_dispatcher_backend) == expected
    print("KIMI_K2_SMOKE_PASS " + json.dumps({
        "arm": arm, "layers": model.num_layers, "hidden_units": model.hidden_size,
        "experts": model.num_moe_experts, "topk": model.moe_router_topk,
        "dispatcher": expected[0], "backend": expected[1],
    }, sort_keys=True))
